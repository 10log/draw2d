 ...
 * Additionally the anchor resolves conflicts by spreading the anchor if more than one
 * connection has the same reference point.
 * In the case of a DirectRouter parallel connections are the result.
 */
export class FanConnectionAnchor extends ConnectionAnchor {
  /**
   * Creates a new FanConnectionAnchor
   * @param owner The figure to use for the anchor calculation
   * @param separation The separation or fan distance between the concurrent/conflicting anchors
   */
  constructor(owner: Figure, separation?: number);

  /** The separation distance between concurrent anchors */
  separation: number;
